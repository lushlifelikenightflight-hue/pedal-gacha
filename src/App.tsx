...
function SharePanel({ pedal, sourceImage, editorialTopImage, onNotice }: { pedal: Pedal; sourceImage: string; editorialTopImage: string; onNotice: (message: string) => void }) {
  ...
  const pageUrl = typeof window === 'undefined' ? 'https://lushlifelikenightflight-hue.github.io/pedal-gacha/' : new URL('./', window.location.href).toString();
  const baseText = `PEDAL FORGEで「${pedal.name.replace(' // LIMITED', '')}」を錬成しました。\n\n${pedal.type}\n\n${pageUrl}`; const [postText, setPostText] = useState(baseText); const [preview, setPreview] = useState(''); const [imageBlob, setImageBlob] = useState<Blob | null>(null); const [busy, setBusy] = useState(false);
  ...
  const openX = async () => { setBusy(true); try { const file = await makeFile(); if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) { try { await navigator.share({ files: [file], text: completeText, title: `${pedal.name} / PEDAL FORGE` }); onNotice('シェア画像と投稿文を共有画面へ送りました'); return; } catch (error) { if (error instanceof DOMException && error.name === 'AbortError') return; } } try { await navigator.clipboard.writeText(completeText); } catch { /* X intent still carries the text */ } try { await copyImage(); } catch { await saveImage(); } const url = new URL('https://x.com/intent/post'); url.searchParams.set('text', completeText); window.open(url.toString(), '_blank', 'noopener,noreferrer'); onNotice('画像をコピーまたは保存しました。Xの投稿へ添付してください。'); } finally { setBusy(false); } };
