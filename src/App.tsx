...
type PedalBoardBackground = 'wood' | 'tile' | 'paper';
...
const boardBackgroundColors: Record<PedalBoardBackground, { scene: string; surface: string }> = {
  wood: { scene: '#d9c9ad', surface: '#b9824d' }, tile: { scene: '#e1e0db', surface: '#c9c7c0' }, paper: { scene: '#f2f0e8', surface: '#e5e1d6' },
};const boardBackgroundLabels: Record<PedalBoardBackground, string> = { wood: 'WOOD', tile: 'TILE', paper: 'PAPER' };
...
function PedalBoardScene({ pedals, finishes, background }: { pedals: Pedal[]; finishes: Record<string, PedalFinish>; background: PedalBoardBackground }) {
  ...
  return <><color attach="background" args={[colors.scene]} /><ambientLight intensity={.72} /><hemisphereLight color="#fffaf0" groundColor={background === 'wood' ? '#9b7654' : '#9b9b96'} intensity={1.55} /><directionalLight position={[-5, 11, 7]} intensity={4.2} color="#fff8e9" castShadow /><directionalLight position={[7, 5, -5]} intensity={1.5} color="#e8f0ff" />
...
function wideBoardBlob(source: HTMLCanvasElement, background: PedalBoardBackground) {
  const canvas = document.createElement('canvas'); canvas.width = 1600; canvas.height = 900; const context = canvas.getContext('2d'); if (!context) return Promise.reject(new Error('CANVAS_UNAVAILABLE')); context.fillStyle = boardBackgroundColors[background].scene; context.fillRect(0, 0, canvas.width, canvas.height); const scale = Math.min(canvas.width / source.width, canvas.height / source.height); const width = source.width * scale; const height = source.height * scale; context.drawImage(source, (canvas.width - width) / 2, (canvas.height - height) / 2, width, height); return canvasBlob(canvas);
}
function BoardCaptureBridge({ captureRef, background }: { captureRef: React.MutableRefObject<(() => Promise<Blob>) | null>; background: PedalBoardBackground }) {
  const { gl, scene, camera } = useThree();
  useEffect(() => { captureRef.current = () => { gl.render(scene, camera); return wideBoardBlob(gl.domElement, background); }; return () => { captureRef.current = null; }; }, [background, camera, captureRef, gl, scene]);
  return null;
}
function PedalBoardCanvas({ pedals, finishes, background, captureRef }: { pedals: Pedal[]; finishes: Record<string, PedalFinish>; background: PedalBoardBackground; captureRef: React.MutableRefObject<(() => Promise<Blob>) | null> }) {
  return <div className={'pedalboard-preview background-' + background}><Canvas shadows dpr={[1, 1.5]} gl={{ preserveDrawingBuffer: true, antialias: true }} camera={{ position: [0, 10, 7], fov: 34, near: .01, far: 120 }}><RenderSettings viewMode="stage" /><BoardCaptureBridge captureRef={captureRef} background={background} /><PedalBoardScene pedals={pedals} finishes={finishes} background={background} /></Canvas><span className="board-signal-flow">INPUT / 01 — RIGHT → LEFT — OUTPUT</span></div>;
...
function ArchiveDrawer({ collection, boards, finishes, archiveImages, selectedBoardId, onSelectedBoard, onClose, onOpenPedal, onDeletePedal, onCreateBoard, onRenameBoard, onDeleteBoard, onAddPedal, onRemovePedal, onMovePedal, onBackground, onNotice }: ArchiveDrawerProps) {
  const [tab, setTab] = useState<'pedals' | 'boards'>('pedals'); const [sharing, setSharing] = useState(false); const [shareTexts, setShareTexts] = useState<Record<string, string>>({}); const boardCaptureRef = useRef<(() => Promise<Blob>) | null>(null); const selected = boards.find(board => board.id === selectedBoardId) || boards[0] || null; const boardPedals = selected ? selected.pedalIds.map(id => collection.find(item => item.id === id)).filter((item): item is Pedal => Boolean(item)) : []; const defaultShareText = selected ? 'PEDAL FORGE / ' + (selected.name.trim() || 'MY EFFECTS BOARD') + (boardPedals.length ? '\n' + boardPedals.map(item => item.name.replace(' // LIMITED', '')).join(' → ') : '') : ''; const shareText = selected ? shareTexts[selected.id] ?? defaultShareText : '';
...
export default function App() {
  ...
  const createBoard = () => { if (boards.length >= 3) return setNotice('ボードは1ユーザーにつき3セットまでです'); const board: PedalBoard = { id: 'board-' + Date.now() + '-' + Math.random().toString(36).slice(2, 6), name: 'BOARD ' + (boards.length + 1), pedalIds: [], background: 'wood' }; setBoards(current => [...current, board].slice(0, 3)); setSelectedBoardId(board.id); };
