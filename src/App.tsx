...
type SignatureFontCategory = 'yomogi' | 'hachi-maru-pop' | 'zen-kurenaido' | 'klee-one' | 'kaisei-decol' | 'caveat' | 'pacifico' | 'dancing-script' | 'sacramento' | 'great-vibes';
...
const japaneseSignatureFontOptions: Array<[SignatureFontCategory, string]> = [['yomogi', 'Yomogi / よもぎ'], ['hachi-maru-pop', 'Hachi Maru Pop / はちまるポップ'], ['zen-kurenaido', 'Zen Kurenaido'], ['klee-one', 'Klee One'], ['kaisei-decol', 'Kaisei Decol / 解星デコール']];
const latinSignatureFontOptions: Array<[SignatureFontCategory, string]> = [['caveat', 'Caveat'], ['pacifico', 'Pacifico'], ['dancing-script', 'Dancing Script'], ['sacramento', 'Sacramento'], ['great-vibes', 'Great Vibes']];
const signatureFontOptions = [...japaneseSignatureFontOptions, ...latinSignatureFontOptions];
const signatureFontIds = new Set<SignatureFontCategory>(signatureFontOptions.map(([value]) => value));
const alignmentLabels: Record<AlignmentMode, string> = { left: '左寄せ', 'center-x': '左右中央', right: '右寄せ', top: '上寄せ', 'center-y': '上下中央', bottom: '下寄せ', center: '面の中央' };
...
const markFonts: Record<PedalMark['font'], string> = {
  yomogi: '"Yomogi", "Yu Kyokasho", cursive', 'hachi-maru-pop': '"Hachi Maru Pop", "Hiragino Maru Gothic ProN", cursive', 'zen-kurenaido': '"Zen Kurenaido", "Yu Gothic", sans-serif', 'klee-one': '"Klee One", "Yu Mincho", serif', 'kaisei-decol': '"Kaisei Decol", "Yu Mincho", serif',
  caveat: '"Caveat", cursive', pacifico: '"Pacifico", cursive', 'dancing-script': '"Dancing Script", cursive', sacramento: '"Sacramento", cursive', 'great-vibes': '"Great Vibes", cursive',
};
function markTexture(mark: PedalMark) {
  const canvas = document.createElement('canvas'); canvas.width = 1024; canvas.height = 260; const ctx = canvas.getContext('2d')!; ctx.clearRect(0, 0, 1024, 260); ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.font = `400 132px ${markFonts[mark.font]}`; ctx.fillStyle = mark.color; ctx.fillText(mark.text, 512, 130);
  const texture = new THREE.CanvasTexture(canvas); texture.colorSpace = THREE.SRGBColorSpace; texture.anisotropy = 4; return texture;
}
function SignatureMark({ mark, size, surfaceY }: { mark: PedalMark; size: { width: number; height: number; depth: number }; surfaceY: number }) {
  ...
  useEffect(() => { let active = true; void document.fonts.load(`400 132px ${markFonts[mark.font]}`, mark.text || '署名').then(() => { if (active) setFontRevision(value => value + 1); }).catch(() => undefined); return () => { active = false; }; }, [mark.font, mark.text]);
  const texture = useMemo(() => markTexture(mark), [mark, fontRevision]); useEffect(() => () => texture.dispose(), [texture]); if (!mark.enabled || !mark.text.trim()) return null;
...
const createDefaultMark = (_surface: MarkSurface = 'top'): PedalMark => ({ id: 'signature-top', enabled: false, text: '', surface: 'top', u: .5, v: .5, size: .24, rotation: -5, font: 'yomogi', color: '#f4f1e8', style: 'print' });
...
function FinishEditor({ mark, graphic, stickers, error, tool, open, canUndo, canRedo, onTool, onClose, onMarkChange, onMarkRemove, onGraphicFile, onGraphicChange, onGraphicRemove, onGraphicSelect, onInspect, onUndo, onRedo, onReset }: { mark: PedalMark; graphic: UserGraphic | null; stickers: UserGraphic[]; error: string; tool: 'signature' | 'sticker'; open: boolean; canUndo: boolean; canRedo: boolean; onTool: (tool: 'signature' | 'sticker') => void; onClose: () => void; onMarkChange: (mark: PedalMark) => void; onMarkRemove: () => void; onGraphicFile: (file: File, replace: boolean) => void; onGraphicChange: (patch: Partial<UserGraphic>) => void; onGraphicRemove: () => void; onGraphicSelect: (id: string) => void; onInspect: (surface: MarkSurface) => void; onUndo: () => void; onRedo: () => void; onReset: () => void }) {
  ...
  return <section className="signature-editor direct-3d-mark-controls">
    ...
    <nav className="finish-tool-tabs" aria-label="編集する加工"><button type="button" className={tool === 'signature' ? 'active' : ''} onClick={() => { onTool('signature'); onInspect('top'); }}>署名</button><button type="button" className={tool === 'sticker' ? 'active' : ''} onClick={() => { onTool('sticker'); onInspect('top'); }}>ステッカー</button></nav>
    {tool === 'signature' ? <>
      ...
      <div className="direct-mark-settings">
        <label>フォント<select value={mark.font} onChange={event => onMarkChange({ ...mark, font: event.target.value as PedalMark['font'] })}><optgroup label="日本語フォント">{japaneseSignatureFontOptions.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</optgroup><optgroup label="英字フォント">{latinSignatureFontOptions.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</optgroup></select></label>
        <label>COLOR<input type="color" value={mark.color} onChange={event => onMarkChange({ ...mark, color: event.target.value })} /></label>
      </div>
      <div className="decoration-align" aria-label="署名の位置を揃える"><span>位置を揃える</span>{(['left', 'center-x', 'right', 'top', 'center-y', 'bottom', 'center'] as AlignmentMode[]).map(mode => <button type="button" key={mode} onClick={() => align(mode)}>{alignmentLabels[mode]}</button>)}</div>
      <p className="direct-mark-status">天面上の枠：4隅でサイズ / 上で角度 · 中央付近は自動スナップ</p>
      ...
      <div className="sticker-library"><header><b>STICKERS</b><span>{stickers.length} / 5</span><label className="change-graphic">＋ 追加{fileInput(false)}</label></header>{stickers.length > 0 && <div>{stickers.map(item => <button type="button" key={item.id} className={graphic?.id === item.id ? 'active' : ''} onClick={() => { onGraphicSelect(item.id); onInspect('top'); }}><img src={item.textureUrl || item.sourceUrl} alt="" /><span>{item.fileName}</span><small>天面</small></button>)}</div>}</div>
      {!graphic ? <label className="graphic-drop sticker-drop">{fileInput(false)}<strong>＋ ステッカー画像を選択</strong><span>PNG / JPG / WEBP · 最大15MB · 512px以下へ自動圧縮</span></label> : <>
        ...
        <div className="decoration-align" aria-label="ステッカーの位置を揃える"><span>位置を揃える</span>{(['left', 'center-x', 'right', 'top', 'center-y', 'bottom', 'center'] as AlignmentMode[]).map(mode => <button type="button" key={mode} onClick={() => align(mode)}>{alignmentLabels[mode]}</button>)}</div>
      </>}
      {error && <p className="graphic-error" role="alert">{error}</p>}
      <p className="direct-mark-status">天面上の枠：4隅でサイズ / 上で角度 · 天面より大きくでき、はみ出しは天面でクロップ</p>
...
const normalizeSurface = (_surface: unknown): MarkSurface => 'top';
const normalizeSignatureFont = (font: unknown): SignatureFontCategory => signatureFontIds.has(font as SignatureFontCategory) ? font as SignatureFontCategory : typeof font === 'string' && !font.endsWith('-jp') && !['mincho-jp', 'gyosho-jp', 'brush-jp', 'handwritten-jp', 'retro-jp', 'seal-jp'].includes(font) ? 'caveat' : 'yomogi';
const normalizeMark = (value: Partial<PedalMark>): PedalMark => ({ ...createDefaultMark(), ...value, id: 'signature-top', surface: 'top', font: normalizeSignatureFont(value.font), style: 'print' });
const normalizeSignatures = (values: Partial<PedalMark>[]): PedalMark[] => { const selected = values.find(value => value.surface === 'top') || values.find(value => value.enabled && value.text?.trim()) || values[0]; return selected ? [normalizeMark(selected)] : []; };
...
const storedFinishes = (): Record<string, PedalFinish> => { try { const current = JSON.parse(localStorage.getItem('pedal-gacha-finish-v3') || '{}') as Record<string, Partial<PedalFinish>>; if (Object.keys(current).length) return Object.fromEntries(Object.entries(current).map(([id, finish]) => [id, { signatures: normalizeSignatures(finish.signatures || []), stickers: (finish.stickers || []).map(graphic => normalizeGraphic(graphic)) }])); const legacy = JSON.parse(localStorage.getItem('pedal-gacha-marks-v1') || '{}') as Record<string, Partial<PedalMark>>; return Object.fromEntries(Object.entries(legacy).map(([id, mark]) => [id, { signatures: normalizeSignatures([mark]), stickers: [] }])); } catch { return {}; } };
...
export default function App() {
  ...
  const changeMark = (nextValue: PedalMark) => { const next = normalizeMark(nextValue); commitFinish(finish => ({ ...finish, signatures: [...finish.signatures.filter(item => item.id !== next.id && item.surface !== next.surface), next] })); setInspectSurface(next.surface); };
  ...
  const pdf = async () => {
    ...
    activeFinish.stickers.forEach(graphic => lines.push(`STICKER  ${graphic.fileName} / ${graphic.surface.toUpperCase()} / ${Math.round(graphic.size * 100)}%`)); activeFinish.signatures.filter(mark => mark.enabled && mark.text.trim()).forEach(mark => lines.push(`OWNER MARK  ${mark.text} / TOP`));
  ...
  return <main>
    ...
    <section id="forging-stage" ref={stageRef} className={'stage-wrap phase-' + phase + ' view-' + viewMode + (workflow === 'forged' || workflow === 'finishing' ? ' has-finish-rail' : '') + (workflow === 'finishing' ? ' is-editing' : '')}>
      <div className="workflow-location"><span>02 / FORGE</span><b>錬成</b></div><div className="stage-label">{phase === 'idle' ? 'NO PEDAL YET / SELECT PARAMETERS' : workflow === 'finishing' ? 'FINISH EDITOR / TOP FACE LOCKED' : phase === 'result' ? 'ALCHEMY COMPLETE / DRAG 360° / WHEEL TO ZOOM' : forgeStep}</div>
      ...
      {phase === 'result' && <div className="inspection-frame" aria-hidden="true">
        <div className="inspection-frame-head"><b>{workflow === 'finishing' ? 'TOP FACE LOCK' : '360° VIEW'}</b><span>{workflow === 'finishing' ? 'SIGNATURE / STICKER ONLY' : '↔ DRAG TO ROTATE'}</span></div>
