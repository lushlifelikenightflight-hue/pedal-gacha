...
function BoardWoodFloor({ width, depth }: { width: number; depth: number }) {
  const plankCount = Math.max(6, Math.ceil(depth / .72)); const grainCount = Math.max(12, Math.ceil(width * 2.2));
  return <group>
    <mesh position={[0, -.2, 0]} receiveShadow><boxGeometry args={[width, .4, depth]} /><meshPhysicalMaterial color="#b9824d" roughness={.72} metalness={.02} clearcoat={.12} clearcoatRoughness={.68} /></mesh>
...
function PedalBoardScene({ pedals, finishes }: { pedals: Pedal[]; finishes: Record<string, PedalFinish> }) {
  ...
  return <><color attach="background" args={['#d9c9ad']} /><ambientLight intensity={.72} /><hemisphereLight color="#fffaf0" groundColor="#9b7654" intensity={1.55} /><directionalLight position={[-5, 11, 7]} intensity={4.2} color="#fff8e9" castShadow /><directionalLight position={[7, 5, -5]} intensity={1.5} color="#e8f0ff" />
    ...
    <BoardWoodFloor width={layout.width} depth={layout.depth} />
