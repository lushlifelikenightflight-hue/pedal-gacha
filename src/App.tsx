...
type PedalBoard = { id: string; name: string; pedalIds: string[] };
...
function createMaskedStickerTexture(surfaceWidth: number, surfaceHeight: number, cornerRadius: number) {
  const canvas = document.createElement('canvas'); canvas.width = 512; canvas.height = Math.max(64, Math.round(512 * surfaceHeight / surfaceWidth));
...
function compressStickerFile(file: File): Promise<{ dataUrl: string; width: number; height: number; mimeType: string }> {
  return new Promise((resolve, reject) => {
    ...
    image.onload = () => {
      ...
      const limit = 512; const ratio = Math.min(1, limit / Math.max(image.naturalWidth, image.naturalHeight)); const width = Math.max(1, Math.round(image.naturalWidth * ratio)); const height = Math.max(1, Math.round(image.naturalHeight * ratio));
...
async function createPedalboardShare(board: PedalBoard, pedals: Pedal[]): Promise<Blob> {
  const canvas = document.createElement('canvas'); canvas.width = 1600; canvas.height = 900; const ctx = canvas.getContext('2d'); if (!ctx) throw new Error('CANVAS_UNAVAILABLE');
  const name = board.name.trim() || 'UNTITLED BOARD'; ctx.fillStyle = '#0b0f0b'; ctx.fillRect(0, 0, canvas.width, canvas.height);
  ctx.fillStyle = '#b7ff19'; ctx.font = '800 24px Arial'; ctx.fillText('PEDAL FORGE / EFFECTS BOARD', 78, 72);
  ctx.fillStyle = '#f2f4ee'; drawFittedCanvasText(ctx, name, 78, 144, 1444, 70, 38, 'Arial');
  ctx.fillStyle = '#171d17'; ctx.beginPath(); ctx.roundRect(70, 200, 1460, 570, 36); ctx.fill(); ctx.strokeStyle = '#455044'; ctx.lineWidth = 4; ctx.stroke();
  if (!pedals.length) { ctx.fillStyle = '#7f897b'; ctx.font = '700 30px Arial'; ctx.textAlign = 'center'; ctx.fillText('NO PEDALS IN THIS BOARD', 800, 500); ctx.textAlign = 'left'; }
  const gap = 24; const slotWidth = (1370 - gap * Math.max(0, pedals.length - 1)) / Math.max(1, pedals.length);
  pedals.forEach((item, index) => {
    const enclosure = enclosureDimensions[item.enclosure]; const maxWidth = Math.min(238, slotWidth - 12); const maxHeight = 390; const scale = Math.min(maxWidth / enclosure.width, maxHeight / enclosure.height); const width = enclosure.width * scale; const height = enclosure.height * scale; const x = 115 + index * (slotWidth + gap) + (slotWidth - width) / 2; const y = 275 + (maxHeight - height) / 2;
    ctx.fillStyle = '#050705'; ctx.fillRect(x - 10, y + 12, width + 20, height + 18); ctx.fillStyle = item.palette[1]; ctx.strokeStyle = item.palette[0]; ctx.lineWidth = 8; ctx.beginPath(); ctx.roundRect(x, y, width, height, 18); ctx.fill(); ctx.stroke();
    const controlCount = Math.max(2, Math.min(8, item.knobs.length || 2)); const columns = Math.min(4, controlCount); const rows = Math.ceil(controlCount / columns); const knobRadius = Math.max(10, Math.min(24, width / 11));
    for (let control = 0; control < controlCount; control++) { const column = control % columns; const row = Math.floor(control / columns); const cx = x + width * (.18 + column * (.64 / Math.max(1, columns - 1))); const cy = y + height * (.18 + row * (.2 / Math.max(1, rows))); ctx.fillStyle = item.hardwareColors?.[control] || '#20231f'; ctx.beginPath(); ctx.arc(cx, cy, knobRadius, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = '#d9ded4'; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(cx, cy - knobRadius * .65); ctx.stroke(); }
    ctx.fillStyle = item.graphicColor || item.palette[0]; ctx.font = '900 ' + Math.max(14, Math.min(28, width / 8)) + 'px Arial'; ctx.textAlign = 'center'; const label = item.name.replace(' // LIMITED', ''); ctx.fillText(label.length > 18 ? label.slice(0, 17) + '…' : label, x + width / 2, y + height * .64, width - 18);
    ctx.fillStyle = '#dce2d8'; ctx.font = '700 15px Arial'; ctx.fillText(item.type.toUpperCase(), x + width / 2, y + height * .71, width - 18); ctx.fillStyle = '#b8beb4'; ctx.beginPath(); ctx.arc(x + width / 2, y + height * .87, Math.min(24, width * .1), 0, Math.PI * 2); ctx.fill(); ctx.textAlign = 'left';
  });
  ctx.fillStyle = '#879181'; ctx.font = '700 20px Arial'; ctx.fillText(String(pedals.length) + ' / 5 PEDALS', 78, 838); ctx.textAlign = 'right'; ctx.fillText('MOONLIT CIRCUIT DEVICES', 1522, 838); ctx.textAlign = 'left'; return canvasBlob(canvas);
}
type ArchiveDrawerProps = {
  collection: Pedal[]; boards: PedalBoard[]; selectedBoardId: string; onSelectedBoard: (id: string) => void; onClose: () => void; onOpenPedal: (pedal: Pedal) => void; onDeletePedal: (pedal: Pedal) => void; onCreateBoard: () => void; onRenameBoard: (id: string, name: string) => void; onDeleteBoard: (board: PedalBoard) => void; onAddPedal: (boardId: string, pedalId: string) => void; onRemovePedal: (boardId: string, pedalId: string) => void; onMovePedal: (boardId: string, pedalId: string, direction: -1 | 1) => void; onNotice: (message: string) => void;
};
function ArchiveDrawer({ collection, boards, selectedBoardId, onSelectedBoard, onClose, onOpenPedal, onDeletePedal, onCreateBoard, onRenameBoard, onDeleteBoard, onAddPedal, onRemovePedal, onMovePedal, onNotice }: ArchiveDrawerProps) {
  const [tab, setTab] = useState<'pedals' | 'boards'>('pedals'); const [sharing, setSharing] = useState(false); const selected = boards.find(board => board.id === selectedBoardId) || boards[0] || null; const boardPedals = selected ? selected.pedalIds.map(id => collection.find(item => item.id === id)).filter((item): item is Pedal => Boolean(item)) : [];
  const shareBoard = async () => {
    if (!selected || !boardPedals.length) return; setSharing(true);
    try { const blob = await createPedalboardShare(selected, boardPedals); const file = new File([blob], (selected.name.trim() || 'pedal-board').replace(/[\\/:*?"<>|]+/g, '-') + '.png', { type: 'image/png' }); if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) { await navigator.share({ files: [file], title: selected.name || 'PEDAL FORGE BOARD', text: 'PEDAL FORGE / ' + (selected.name || 'MY EFFECTS BOARD') }); onNotice('ボード全体の画像を共有画面へ渡しました'); } else { downloadShareFile(file); onNotice('共有画像を保存しました'); } }
  ...
  return <div className="drawer-backdrop" role="presentation" onMouseDown={onClose}><aside className="drawer archive-drawer" role="dialog" aria-modal="true" aria-label="錬成済みペダル保管庫" onMouseDown={event => event.stopPropagation()}><button className="close" onClick={onClose} aria-label="保管庫を閉じる">×</button><p className="eyebrow">FORGED ARCHIVE</p><h2>ARCHIVE</h2>
    ...
    {tab === 'pedals' ? <section className="archive-section"><header><b>FORGED PEDALS</b><span>{collection.length} UNITS</span></header>{collection.length ? <div className="archive-pedal-list">{collection.map(item => <article className="archive-pedal-card" key={item.id}><button type="button" className="archive-open" onClick={() => onOpenPedal(item)}><i style={{ background: item.palette[0] }} /><span><b>{item.name}</b><small>{item.brand?.manufacturerName || item.brandLabel || 'FURNACE AUDIO WORKS'} / {item.modelNumber || item.serial}</small></span></button><button type="button" className="archive-delete" onClick={() => onDeletePedal(item)}>削除</button></article>)}</div> : <p className="empty">保存済みの個体はありません。完成画面の最後にある「アーカイブへ保存」から追加できます。</p>}</section> :
    <section className="archive-section board-builder"><header><b>EFFECTS BOARDS</b><span>{boards.length} / 3 SETS</span></header><div className="board-tabs">{boards.map(board => <button type="button" key={board.id} className={selected?.id === board.id ? 'active' : ''} onClick={() => onSelectedBoard(board.id)}>{board.name || 'UNTITLED'}</button>)}<button type="button" onClick={onCreateBoard} disabled={boards.length >= 3}>＋ 新規セット</button></div>
      {selected ? <><div className="board-editor-head"><label>SET NAME<input value={selected.name} maxLength={32} onChange={event => onRenameBoard(selected.id, event.target.value)} /></label><button type="button" className="archive-delete" onClick={() => onDeleteBoard(selected)}>セットを削除</button></div><div className="pedalboard-preview">{boardPedals.length ? boardPedals.map((item, index) => <article className="board-pedal" key={item.id} style={{ '--pedal-color': item.palette[1], '--pedal-accent': item.palette[0] } as React.CSSProperties}><div><span>{index + 1}</span><b>{item.name}</b><small>{item.type}</small></div><nav><button type="button" onClick={() => onMovePedal(selected.id, item.id, -1)} disabled={index === 0} aria-label="左へ移動">←</button><button type="button" onClick={() => onMovePedal(selected.id, item.id, 1)} disabled={index === boardPedals.length - 1} aria-label="右へ移動">→</button><button type="button" onClick={() => onRemovePedal(selected.id, item.id)}>外す</button></nav></article>) : <p>下の保存済みペダルから追加してください。</p>}</div><div className="board-actions"><span>{boardPedals.length} / 5 PEDALS</span><button type="button" className="share-primary" onClick={shareBoard} disabled={!boardPedals.length || sharing}>{sharing ? '画像を生成中…' : 'このボード全体をシェア'}</button></div><div className="board-pedal-picker"><b>保存済みペダルを追加</b>{collection.map(item => { const added = selected.pedalIds.includes(item.id); return <button type="button" key={item.id} disabled={added || selected.pedalIds.length >= 5} onClick={() => onAddPedal(selected.id, item.id)}><span>{item.name}</span><small>{added ? '追加済み' : selected.pedalIds.length >= 5 ? '5台まで' : '＋ 追加'}</small></button>; })}</div></> : <p className="empty">「新規セット」からエフェクターボードを作成してください。</p>}</section>}
  </aside></div>;
}
const storedBoards = (): PedalBoard[] => { try { const value = JSON.parse(localStorage.getItem('pedal-gacha-boards-v1') || '[]'); if (!Array.isArray(value)) return []; return value.slice(0, 3).map((board, index) => ({ id: typeof board.id === 'string' ? board.id : 'board-' + index, name: typeof board.name === 'string' ? board.name.slice(0, 32) : 'BOARD ' + (index + 1), pedalIds: Array.isArray(board.pedalIds) ? board.pedalIds.filter((id: unknown): id is string => typeof id === 'string').slice(0, 5) : [] })); } catch { return []; } };
...
export default function App() {
  ...
  const createBoard = () => { if (boards.length >= 3) return setNotice('ボードは1ユーザーにつき3セットまでです'); const board = { id: 'board-' + Date.now() + '-' + Math.random().toString(36).slice(2, 6), name: 'BOARD ' + (boards.length + 1), pedalIds: [] as string[] }; setBoards(current => [...current, board].slice(0, 3)); setSelectedBoardId(board.id); };
  ...
  return <main>
    ...
    {phase === 'result' && pedal && !editorialCoverImage && <EditorialCoverCapture key={`cover-${pedal.id}-${viewReset}`} pedal={pedal} stickers={activeFinish.stickers} signatures={activeFinish.signatures} onCapture={setEditorialCoverImage} />}
    {phase === 'result' && pedal && !editorialTopImage && <EditorialTopCapture key={`top-${pedal.id}-${viewReset}`} pedal={pedal} stickers={activeFinish.stickers} signatures={activeFinish.signatures} onCapture={setEditorialTopImage} />}
    ...
    {drawer && <ArchiveDrawer collection={collection} boards={boards} selectedBoardId={selectedBoardId} onSelectedBoard={setSelectedBoardId} onClose={() => setDrawer(false)} onOpenPedal={openArchivedPedal} onDeletePedal={deleteArchivedPedal} onCreateBoard={createBoard} onRenameBoard={renameBoard} onDeleteBoard={deleteBoard} onAddPedal={addPedalToBoard} onRemovePedal={removePedalFromBoard} onMovePedal={moveBoardPedal} onNotice={setNotice} />}
