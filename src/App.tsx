...
function PedalBoardCanvas({ pedals, finishes, background, boardName, captureRef }: { pedals: Pedal[]; finishes: Record<string, PedalFinish>; background: PedalBoardBackground; boardName: string; captureRef: React.MutableRefObject<(() => Promise<Blob>) | null> }) {
  return <div className={'pedalboard-preview background-' + background}><Canvas shadows dpr={[1, 1.5]} gl={{ preserveDrawingBuffer: true, antialias: true }} camera={{ position: [0, 10, 7], fov: 34, near: .01, far: 120 }}><RenderSettings viewMode="stage" /><BoardCaptureBridge captureRef={captureRef} background={background} boardName={boardName} pedalCount={pedals.length} /><PedalBoardScene pedals={pedals} finishes={finishes} background={background} /></Canvas><span className="board-signal-flow">INPUT / 01 — RIGHT → LEFT — OUTPUT</span></div>;
