...
function PedalBoardScene({ pedals, finishes }: { pedals: Pedal[]; finishes: Record<string, PedalFinish> }) {
  const layout = useMemo(() => createPedalBoardLayout(pedals), [pedals]); const boardTop = 0;
  return <><color attach="background" args={['#0a0d0a']} /><ambientLight intensity={.38} /><hemisphereLight color="#ffffff" groundColor="#111611" intensity={1.15} /><directionalLight position={[-5, 11, 7]} intensity={3.6} color="#fff5e7" castShadow /><directionalLight position={[7, 5, -5]} intensity={1.25} color="#dce8ff" />
    <CameraController enabled home={[0, 1.35, 1]} resetToken={pedals.length} autoRotate={false} fitFrame={{ width: layout.width, height: layout.depth, padding: 1.2 }} />
    <mesh position={[0, -.2, 0]} receiveShadow><boxGeometry args={[layout.width, .4, layout.depth]} /><meshPhysicalMaterial color="#171b17" roughness={.78} metalness={.18} clearcoat={.18} /></mesh>
    {[-.31, 0, .31].map(position => <mesh key={position} position={[0, .015, position * layout.depth]} receiveShadow><boxGeometry args={[layout.width * .94, .025, .34]} /><meshStandardMaterial color="#080a08" roughness={.95} /></mesh>)}
    {layout.items.map(item => { const finish = finishes[item.pedal.id] || emptyFinish(); return <group key={item.pedal.id} position={[item.x, boardTop + item.depth / 2 + .035, 0]}><PedalModel pedal={item.pedal} runtimeMode="play" userGraphics={finish.stickers} marks={finish.signatures} shotPreset="editorial-cover-top" /></group>; })}
...
function BoardCaptureBridge({ captureRef }: { captureRef: React.MutableRefObject<(() => Promise<Blob>) | null> }) {
  const { gl, scene, camera } = useThree();
  useEffect(() => { captureRef.current = () => { gl.render(scene, camera); return canvasBlob(gl.domElement); }; return () => { captureRef.current = null; }; }, [camera, captureRef, gl, scene]);
  return null;
}
function PedalBoardCanvas({ pedals, finishes, captureRef }: { pedals: Pedal[]; finishes: Record<string, PedalFinish>; captureRef: React.MutableRefObject<(() => Promise<Blob>) | null> }) {
  return <div className="pedalboard-preview"><Canvas shadows dpr={[1, 1.5]} gl={{ preserveDrawingBuffer: true, antialias: true }} camera={{ position: [0, 10, 7], fov: 34, near: .01, far: 120 }}><RenderSettings viewMode="stage" /><BoardCaptureBridge captureRef={captureRef} /><PedalBoardScene pedals={pedals} finishes={finishes} /></Canvas><span className="board-signal-flow">INPUT / 01 — RIGHT → LEFT — OUTPUT</span></div>;
}
...
function ArchiveDrawer({ collection, boards, finishes, archiveImages, selectedBoardId, onSelectedBoard, onClose, onOpenPedal, onDeletePedal, onCreateBoard, onRenameBoard, onDeleteBoard, onAddPedal, onRemovePedal, onMovePedal, onNotice }: ArchiveDrawerProps) {
  const [tab, setTab] = useState<'pedals' | 'boards'>('pedals'); const [sharing, setSharing] = useState(false); const boardCaptureRef = useRef<(() => Promise<Blob>) | null>(null); const selected = boards.find(board => board.id === selectedBoardId) || boards[0] || null; const boardPedals = selected ? selected.pedalIds.map(id => collection.find(item => item.id === id)).filter((item): item is Pedal => Boolean(item)) : [];
  const shareBoard = async () => {
    if (!selected || !boardPedals.length || !boardCaptureRef.current) return; setSharing(true);
    try { const blob = await boardCaptureRef.current(); const file = new File([blob], (selected.name.trim() || 'pedal-board').replace(/[\/:*?"<>|]+/g, '-') + '.png', { type: 'image/png' }); if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) { await navigator.share({ files: [file], title: selected.name || 'PEDAL FORGE BOARD', text: 'PEDAL FORGE / ' + (selected.name || 'MY EFFECTS BOARD') }); onNotice('3Dボード全体の画像を共有画面へ渡しました'); } else { downloadShareFile(file); onNotice('3Dボード全体の画像を保存しました'); } }
  ...
  return <div className="drawer-backdrop" role="presentation" onMouseDown={onClose}><aside className="drawer archive-drawer" role="dialog" aria-modal="true" aria-label="錬成済みペダル保管庫" onMouseDown={event => event.stopPropagation()}><button className="close" onClick={onClose} aria-label="保管庫を閉じる">×</button><p className="eyebrow">FORGED ARCHIVE</p><h2>ARCHIVE</h2>
    ...
    <section className="archive-section board-builder"><header><b>EFFECTS BOARDS / 3D</b><span>{boards.length} / 3 SETS</span></header><div className="board-tabs">{boards.map(board => <button type="button" key={board.id} className={selected?.id === board.id ? 'active' : ''} onClick={() => onSelectedBoard(board.id)}>{board.name || 'UNTITLED'}</button>)}<button type="button" onClick={onCreateBoard} disabled={boards.length >= 3}>＋ 新規セット</button></div>
      {selected ? <><div className="board-editor-head"><label>SET NAME<input value={selected.name} maxLength={32} onChange={event => onRenameBoard(selected.id, event.target.value)} /></label><button type="button" className="archive-delete" onClick={() => onDeleteBoard(selected)}>セットを削除</button></div>{boardPedals.length ? <><PedalBoardCanvas pedals={boardPedals} finishes={finishes} captureRef={boardCaptureRef} /><div className="board-order-strip">{boardPedals.map((item, index) => <article key={item.id}><span>{String(index + 1).padStart(2, '0')} / {index === 0 ? 'INPUT / RIGHT' : index === boardPedals.length - 1 ? 'OUTPUT / LEFT' : 'SIGNAL'}</span><b>{item.name}</b><nav><button type="button" onClick={() => onMovePedal(selected.id, item.id, 1)} disabled={index === boardPedals.length - 1} aria-label="左へ移動">←</button><button type="button" onClick={() => onMovePedal(selected.id, item.id, -1)} disabled={index === 0} aria-label="右へ移動">→</button><button type="button" onClick={() => onRemovePedal(selected.id, item.id)}>外す</button></nav></article>)}</div></> : <div className="pedalboard-preview is-empty"><p>下の保存済みペダルから追加してください。</p></div>}<div className="board-actions"><span>{boardPedals.length} / 5 PEDALS · SIGNAL RIGHT → LEFT</span><button type="button" className="share-primary" onClick={shareBoard} disabled={!boardPedals.length || sharing}>{sharing ? '3D画像を生成中…' : 'この3Dボード全体をシェア'}</button></div><div className="board-pedal-picker"><b>保存済みペダルを追加</b>{collection.map(item => { const added = selected.pedalIds.includes(item.id); return <button type="button" key={item.id} disabled={added || selected.pedalIds.length >= 5} onClick={() => onAddPedal(selected.id, item.id)}><span>{item.name}</span><small>{added ? '追加済み' : selected.pedalIds.length >= 5 ? '5台まで' : '＋ 追加'}</small></button>; })}</div></> : <p className="empty">「新規セット」からエフェクターボードを作成してください。</p>}</section>}
  </aside></div>;
}
...
export default function App() {
  ...
  const storeArchiveThumbnail = (pedalId: string, source: string) => { void compressArchiveThumbnail(source).then(image => setArchiveImages(current => current[pedalId] ? current : { ...current, [pedalId]: image })).catch(() => setNotice('ARCHIVEの天面画像を生成できませんでした')); };
  const saveToArchive = () => { if (!pedal) return; setCollection(current => [pedal, ...current.filter(item => item.id !== pedal.id)].slice(0, 24)); if (editorialTopImage) storeArchiveThumbnail(pedal.id, editorialTopImage); setNotice('このエフェクターをARCHIVEへ保存しました'); };
