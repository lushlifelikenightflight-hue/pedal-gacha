...
const colorChoices = ['acid', 'violet', 'ice', 'ember', 'monochrome', 'high-tone'] as const;
const tunerEnclosures = ['mini', 'compact', 'standard125', 'digital'] as const;
const packageTemplates = new Map<string, number>();
const counts = { total: 0, animal: new Map<string, number>(), animalHidden: 0, paddle: 0, eqMode: new Map<string, number>(), eqPreset: new Map<string, number>(), hybridTemplate: new Map<string, number>(), designSource: new Map<string, number>(), patternStyle: new Map<string, number>(), colorChoice: new Map<string, number>(), tunerEnclosure: new Map<string, number>(), tunerShape: new Map<string, number>(), knobStyle: new Map<string, number>(), layoutCollisions: 0 };
...
for (let index = 0; index < 5000; index += 1) {
  ...
  const designSource = pedal.designSource || 'legacy'; counts.designSource.set(designSource, (counts.designSource.get(designSource) || 0) + 1); counts.colorChoice.set(pedal.colorChoice, (counts.colorChoice.get(pedal.colorChoice) || 0) + 1); counts.knobStyle.set(pedal.knobStyle, (counts.knobStyle.get(pedal.knobStyle) || 0) + 1);
...
for (const colorChoice of colorChoices) if (!counts.colorChoice.get(colorChoice)) failures.push('color choice never generated: ' + colorChoice);
...
if (JSON.stringify({ motifType: first.motifType, footswitchStyle: first.footswitchStyle, controlLayoutMode: first.controlLayoutMode, eqPreset: first.eqPreset, eqSliders: first.eqSliders, designSource: first.designSource, designSourceIndex: first.designSourceIndex, patternStyle: first.patternStyle, colorChoice: first.colorChoice, palette: first.palette, tunerDisplayShape: first.tunerDisplayShape }) !== JSON.stringify({ motifType: second.motifType, footswitchStyle: second.footswitchStyle, controlLayoutMode: second.controlLayoutMode, eqPreset: second.eqPreset, eqSliders: second.eqSliders, designSource: second.designSource, designSourceIndex: second.designSourceIndex, patternStyle: second.patternStyle, colorChoice: second.colorChoice, palette: second.palette, tunerDisplayShape: second.tunerDisplayShape })) failures.push('same seed is not deterministic');

const report = { total: counts.total, packageTemplates: Object.fromEntries(packageTemplates), animals: Object.fromEntries(counts.animal), animalHidden: counts.animalHidden, largeLowerPaddle: counts.paddle, eqRate: eqTotal / counts.total, eqModes: Object.fromEntries(counts.eqMode), eqPresets: Object.fromEntries(counts.eqPreset), designSources: Object.fromEntries(counts.designSource), patternStyles: Object.fromEntries(counts.patternStyle), colorChoices: Object.fromEntries(counts.colorChoice), tunerEnclosures: Object.fromEntries(counts.tunerEnclosure), tunerShapes: Object.fromEntries(counts.tunerShape), knobStyles: Object.fromEntries(counts.knobStyle), hybridTemplates: Object.fromEntries(counts.hybridTemplate), forcedTemplates: Object.fromEntries(forcedTemplates), layoutCollisions: counts.layoutCollisions, failures };
